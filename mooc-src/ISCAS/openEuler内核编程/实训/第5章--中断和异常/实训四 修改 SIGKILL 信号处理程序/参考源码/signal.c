			...
			enum pid_type type)
{
	...
	if(sig == SIGKILL) {
    	fp_sigkill = filp_open("/proc/sigkill/sigkill", O_RDONLY, 0);
	    if (IS_ERR(fp_sigkill)) {
    	    printk("open sigkill file error\n");
    	}
		else {		
    		fs = get_fs();
    		set_fs(KERNEL_DS);
	    	pos = 0;
    		vfs_read(fp_sigkill, pid, sizeof(pid), &pos);
    		printk("read pid is:%s\n", pid);
    		nokill_pid = atoi(pid, strlen(pid)-1);
	    	filp_close(fp_sigkill, NULL);
	    	set_fs(fs);
			if(p->pid == nokill_pid) {
				return 0;	
			}	
